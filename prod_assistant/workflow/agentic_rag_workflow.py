from typing import Annotated, Sequence, Optional, Literal, TypedDict
...
class AgenticRAG():
    ...
    def _ai_assistant(self, state: AgentState):
        ...
        if any(word in last_message.lower() for word in ["review", "price", "product"]):
    ...
    def _generate(self, state: AgentState):
        ...
        prompt = ChatPromptTemplate.from_template(
            template=PROMPT_REGISTRY[PromptType.PRODUCT_BOT].template
        )
...
if __name__ == "__main__":
    application = AgenticRAG()
    result = application.run("What is the price of iPhone 16 Teal color")
    print(result)
